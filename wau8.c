...
#define WAU8_INC_CHEAT_SZ       (1725U)


// relatively prime wheel sizes
// cycle should repeat after 2^63.613300375511121451554498835938
const uint8_t WAU8_WHEEL_SZ[WAU8_KEY_SZ] =
{
    0U,     // 256 (2^8)
    253U,   // 253 (11x23)
    251U,   // 251 (prime)
...
// cheat table for incrementing wheel position and handling wraparound
// without having to do modulo or use an if-block
// wheel 256 has no table since it rolls over from 0 to 255 automatically
const uint8_t WAU8_INC_CHEAT[WAU8_INC_CHEAT_SZ] =
{
// wheel 253
...
{
    size_t ii;
    pcontext->pos[0] = (*pkey)[0];
    for (ii = 1; ii < 8; ii++)
    {
        pcontext->pos[ii] = (*pkey)[ii] % WAU8_WHEEL_SZ[ii];
    }
}
...
{
    pcontext->wheels = pwheels;
}


// advance the wheels
// return 0 when they are all at position 0
uint8_t wau8_advance(wau8_context_t* pcontext)
{
    uint8_t zeropos;

    // lookup operations increment value and perform wraparound
    // cheat offsets:  0U, 253U, 504U, 753U, 1000U, 1245U, 1486U
    pcontext->pos[0]++;
    pcontext->pos[1] = WAU8_INC_CHEAT[pcontext->pos[1]];
    pcontext->pos[2] = WAU8_INC_CHEAT[253U + pcontext->pos[2]];
    pcontext->pos[3] = WAU8_INC_CHEAT[504U + pcontext->pos[3]];
    pcontext->pos[4] = WAU8_INC_CHEAT[753U + pcontext->pos[4]];
    pcontext->pos[5] = WAU8_INC_CHEAT[1000U + pcontext->pos[5]];
    pcontext->pos[6] = WAU8_INC_CHEAT[1245U + pcontext->pos[6]];
    pcontext->pos[7] = WAU8_INC_CHEAT[1486U + pcontext->pos[7]];
...
// returns the current ciphertext byte
uint8_t wau8_get_val(const wau8_context_t * pcontext)
{
    uint8_t result = 0U;
    result ^= (*pcontext->wheels)[pcontext->pos[0]];
    result ^= (*pcontext->wheels)[256U + pcontext->pos[1]];
    result ^= (*pcontext->wheels)[509U + pcontext->pos[2]];
    result ^= (*pcontext->wheels)[760U + pcontext->pos[3]];
    result ^= (*pcontext->wheels)[1009U + pcontext->pos[4]];
    result ^= (*pcontext->wheels)[1256U + pcontext->pos[5]];
    result ^= (*pcontext->wheels)[1501U + pcontext->pos[6]];
    result ^= (*pcontext->wheels)[1742U + pcontext->pos[7]];
    return result;
}
